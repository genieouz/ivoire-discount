...
import { FetchCategoriesForMarketGQL, FetchMainCategoriesForMarketGQL } from 'src/generated/graphql';
...
export class NavService {
	categories: any = [];
	constructor(
		private readonly FetchCategoriesForMarketGQL: FetchCategoriesForMarketGQL,
		private readonly FetchMainCategoriesForMarketGQL: FetchMainCategoriesForMarketGQL
		) {
		// this.categories = this.FetchCategoriesForMarketGQL.fetch({}).subscribe(
		// 	({data}) => {
		// 		this.categories = data.fetchCategoriesForMarket;
		...
		// 		});
		// 		const categoriesMenu = { title: 'Catégories', type: 'sub', active: false, children: items };
		// 		this.MENUITEMS.unshift(categoriesMenu);
		// 		this.items = new BehaviorSubject<Menu[]>(this.MENUITEMS);
		// 	}
		// )
		this.categories = this.FetchMainCategoriesForMarketGQL.fetch({}).subscribe(
			({data}) => {
	...
	MENUITEMS: Menu[] = [
		{ path: `/shop/collection/left/sidebar`, queryParams: { category: 'all' }, title: 'Commerçants', type: 'link' },
		// {
	...
	leftMenuItems = new BehaviorSubject<Menu[]>(this.lm);
